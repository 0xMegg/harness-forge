...
- Stack: Bash, Markdown, Codex Skills
...
## Output Target
- Built templates are deployed to `../Codex-harness-template/`
...
7. Any commit modifying `src/` (outside `src/docs/updates/`) must include `src/docs/updates/<short-hash>.md` + a new INDEX.md row in the same commit (per `src/docs/updates/README.md`). Enforced by `.Codex/hooks/pre-commit-updates-doc-check.sh`.
